...
*   **The Local Connectivity Rule:** **MANDATORY:** Always use `http://127.0.0.1:54321` as the canonical `NEXT_PUBLIC_SUPABASE_URL` in `.env.local`. **NEVER** use `localhost`. The Service Worker uses exact string matching for interception; mixing `127.0.0.1` and `localhost` will bypass the SW, breaking auth caching and offline resilience.
*   **The Stack Health Rule:** **PRIORITY 0 DIAGNOSTIC:** If the application returns a **500 Internal Server Error** on a REST or RPC call in the local environment, the agent MUST immediately run `npx supabase status` to verify that `supabase_rest` and `supabase_auth` are not in a "Stopped" state before investigating application code.
