...
*   **Portal Modals:** Feature dialogs **MUST** use React Portals to render into the `#modal-root` div (provided by `ModalHost` in `layout.tsx`).
    *   **Standard:** Feature components (e.g., `VisitFormModal`, `WineryNoteModal`) own their Portals. `GlobalModalRenderer` is reserved for generic `modalContent` only. Trigger via `useUIStore` triggers, but encapsulate the rendering logic within the feature.
