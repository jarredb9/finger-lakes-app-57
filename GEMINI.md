...
* **The Mutation Settlement Rule:** For asynchronous database actions (Log Visit, Edit Trip, etc.), simply clicking the save button is insufficient. **Standard:** ALWAYS verify the appearance of the success toast using `waitForToast` before proceeding to the next interaction (like closing a modal or verifying a list). This prevents race conditions where the modal unmounts or the store refreshes before the database mutation is fully processed.
...
*   **The Hybrid Click Strategy:** **MANDATORY FOR WEBKIT.** To reliably activate buttons in the container, use a "Hybrid Click" pattern: first attempt `await locator.click({ force: true })`, then immediately follow with a `toPass` retry loop using `robustClick(locator)`. This handles hydration race conditions while ensuring the engine-level click is registered.
*   **The Submission Guard Rule:** E2E tests MUST NOT attempt to click a submission button (Save, Log Visit) if the store's "Saving" state (e.g., `isSavingVisit`) is true. Clicks on disabled buttons cause timeouts. **Standard:** Use `page.evaluate` to verify the store is NOT in a saving state BEFORE attempting the final click.
*   **The Synchronous Guard Rule:** For components triggered by synthetic events or `robustClick`, React's asynchronous `setIsSubmitting` is insufficient. Handlers MUST use a `useRef` guard synchronously at the start of the function to prevent duplicate RPC calls.
...
*   **The Stateful Mocking Rule:** When mocking RPCs in `MockMapsManager` (e.g., `create_trip`, `log_visit`, `delete_visit`), the interceptor MUST update the corresponding `static sharedMockState` (like `sharedMockTrips` or `sharedMockVisits`). **Standard:** Mocked IDs for visits and trips MUST be numeric (integers) to satisfy `parseInt` calls in the store and prevent `400 Bad Request` or `NaN` errors during mutations.
