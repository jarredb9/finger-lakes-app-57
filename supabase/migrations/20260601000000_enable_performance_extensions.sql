...
-- Use a DO block to avoid "must be owner of function" error during local reset
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'subscription_check_filters' AND pronamespace = 'realtime'::regnamespace) THEN
