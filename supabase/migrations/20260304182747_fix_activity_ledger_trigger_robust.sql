-- Robust fix for handle_activity_ledger_entry trigger to prevent field access errors
...
DECLARE
    ...
    v_user_privacy text;
BEGIN
    -- Determine privacy level and winery info for INSERT/UPDATE
    IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
        -- Get user's default privacy level
    ...
    IF TG_OP = 'INSERT' THEN
        IF TG_TABLE_NAME = 'visits' THEN
            ...
            VALUES (
                NEW.user_id, 'visit', NEW.id::text, v_privacy_level, 
                jsonb_build_object(
                    'winery_id', v_winery_id, 'winery_name', v_winery_name,
                    'rating', NEW.rating, 'user_review', NEW.user_review,
        ...
        ELSIF TG_TABLE_NAME = 'favorites' THEN
            INSERT INTO public.activity_ledger (user_id, activity_type, object_id, privacy_level, metadata, created_at)
            VALUES (
                NEW.user_id, 'favorite', NEW.id::text, v_privacy_level, 
                jsonb_build_object('winery_id', v_winery_id, 'winery_name', v_winery_name),
                NEW.created_at
            );
        ELSIF TG_TABLE_NAME = 'wishlist' THEN
            INSERT INTO public.activity_ledger (user_id, activity_type, object_id, privacy_level, metadata, created_at)
            VALUES (
                NEW.user_id, 'wishlist', NEW.id::text, v_privacy_level, 
                jsonb_build_object('winery_id', v_winery_id, 'winery_name', v_winery_name),
                NEW.created_at
            );
        END IF;
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        ELSIF TG_TABLE_NAME = 'favorites' THEN
            UPDATE public.activity_ledger
            SET privacy_level = v_privacy_level,
                metadata = jsonb_build_object('winery_id', v_winery_id, 'winery_name', v_winery_name)
