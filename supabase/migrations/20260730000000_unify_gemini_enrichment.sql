-- Migration: Unify Gemini Enrichment into bulk_upsert_wineries & Remove Review Trigger
-- Date: 2026-07-30
-- Description: Updates bulk_upsert_wineries and RPC functions to handle varietals and vibe_tags, and removes legacy visits review trigger.

-- 1. Drop legacy visits trigger for review-based gemini summaries
...
-- 2. Update bulk_upsert_wineries to include varietals and vibe_tags
...
BEGIN
  FOREACH winery_record IN ARRAY p_wineries_data LOOP
    INSERT INTO public.wineries (
      ...
      varietals = COALESCE(EXCLUDED.varietals, wineries.varietals),
      vibe_tags = CASE 
...
-- 3. Update get_winery_details_by_id to include varietals and vibe_tags
...
CREATE OR REPLACE FUNCTION public.get_winery_details_by_id(p_winery_id integer)
 RETURNS TABLE(id integer, google_place_id text, name text, address text, latitude numeric, longitude numeric, phone text, website text, google_rating numeric, user_rating_count integer, opening_hours jsonb, reviews jsonb, reservable boolean, is_favorite boolean, on_wishlist boolean, user_visited boolean, is_favorite_private boolean, on_wishlist_private boolean, visits jsonb, trip_info jsonb, enrichment_tier text, last_enriched_at timestamp with time zone, generative_summary jsonb, neighborhood_summary jsonb, editorial_summary jsonb, primary_photo_reference text, photo_references jsonb, allows_dogs boolean, good_for_children boolean, outdoor_seating boolean, has_ev_charging boolean, serves_wine boolean, parking_options jsonb, accessibility_flags jsonb, varietals jsonb, vibe_tags text[])
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'auth'
AS $function$ 
...
BEGIN 
  RETURN QUERY SELECT 
    ...
    w.reviews, 
    w.reservable, 
    EXISTS(SELECT 1 FROM public.favorites f WHERE f.winery_id = w.id AND f.user_id = user_uuid) as is_favorite, 
    EXISTS(SELECT 1 FROM public.wishlist wl WHERE wl.winery_id = w.id AND wl.user_id = user_uuid) as on_wishlist, 
    EXISTS(SELECT 1 FROM public.visits v WHERE v.winery_id = w.id AND v.user_id = user_uuid) as user_visited, 
    COALESCE((SELECT f.is_private FROM public.favorites f WHERE f.winery_id = w.id AND f.user_id = user_uuid), false) as is_favorite_private, 
    COALESCE((SELECT wi.is_private FROM public.wishlist wi WHERE wi.winery_id = w.id AND wi.user_id = user_uuid), false) as on_wishlist_private, 
    ( 
      ...
      ), '[]'::jsonb) FROM public.visits v WHERE v.winery_id = w.id AND v.user_id = user_uuid 
    ) as visits, 
    ( 
      SELECT COALESCE(jsonb_agg( 
        jsonb_build_object( 
      ...
      JOIN public.trips t ON tw.trip_id = t.id 
      WHERE tw.winery_id = w.id 
      AND ( 
        t.user_id = user_uuid 
        OR EXISTS ( 
          SELECT 1 FROM public.trip_members tm WHERE tm.trip_id = t.id AND tm.user_id = user_uuid 
        ) 
      ) 
    ...
    w.enrichment_tier, 
    w.last_enriched_at, 
    w.generative_summary, 
    w.neighborhood_summary, 
    w.editorial_summary, 
    w.primary_photo_reference, 
    w.photo_references, 
    w.allows_dogs, 
    w.good_for_children, 
    w.outdoor_seating, 
