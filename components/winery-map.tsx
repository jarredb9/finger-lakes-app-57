...
const SearchUI = memo(({ searchState, searchLocation, setSearchLocation, autoSearch, setAutoSearch, handleSearchSubmit, handleManualSearchArea, filter, onFilterChange, selectedTrip, setSelectedTrip, upcomingTrips }: any) => {
...
function WineryMapLogic({ userId, selectedTrip, setSelectedTrip, selectedWinery, setSelectedWinery, proposedWinery, setProposedWinery }: { userId: string; selectedTrip?: Trip | null; setSelectedTrip: (trip: Trip | null) => void; selectedWinery: Winery | null; setSelectedWinery: (winery: Winery | null) => void; proposedWinery: Winery | null; setProposedWinery: (winery: Winery | null) => void; }) {
...
export default function WineryMapWrapper({ userId }: { userId: string }) {
    ...
    // ** FIX: Added these variables to avoid the ReferenceError. **
    const { favoriteIds, wishlistIds, visitedIds } = useMemo(() => {
        const favoriteIds = new Set(allFavoriteWineries.map(w => w.id));
        const wishlistIds = new Set(allWishlistWineries.map(w => w.id));
        const visitedIds = new Set(allVisitedWineries.map(w => w.id));
    ...
    // ** FIX: Passed all the props to WineryMapLogic. **
    return (
        <APIProvider apiKey={apiKey} libraries={['places', 'geocoding', 'marker']}>
            <WineryMapLogic userId={userId} selectedTrip={selectedTrip} setSelectedTrip={setSelectedTrip} selectedWinery={selectedWinery} setSelectedWinery={setSelectedWinery} proposedWinery={proposedWinery} setProposedWinery={setProposedWinery} trulyDiscoveredWineries={trulyDiscoveredWineries} visitedToRender={visitedToRender} wishlistToRender={wishlistToRender} favoritesToRender={favoritesToRender} />
        </APIProvider>
    );
}

// ** FIX: Updated WineryMapLogic to accept new props. **
function WineryMapLogic({ userId, selectedTrip, setSelectedTrip, selectedWinery, setSelectedWinery, proposedWinery, setProposedWinery, trulyDiscoveredWineries, visitedToRender, wishlistToRender, favoritesToRender }: { userId: string; selectedTrip?: Trip | null; setSelectedTrip: (trip: Trip | null) => void; selectedWinery: Winery | null; setSelectedWinery: (winery: Winery | null) => void; proposedWinery: Winery | null; setProposedWinery: (winery: Winery | null) => void; trulyDiscoveredWineries: Winery[]; visitedToRender: Winery[]; wishlistToRender: Winery[]; favoritesToRender: Winery[]; }) {
  const [searchState, dispatch] = useReducer(searchReducer, initialState);
  const [searchLocation, setSearchLocation] = useState("");
  const [autoSearch, setAutoSearch] = useState(true);
  const [filter, setFilter] = useState<string[]>(['all']);
...
function WineryMapLogic({ userId, selectedTrip, setSelectedTrip, selectedWinery, setSelectedWinery, proposedWinery, setProposedWinery, trulyDiscoveredWineries, visitedToRender, wishlistToRender, favoritesToRender }: { userId: string; selectedTrip?: Trip | null; setSelectedTrip: (trip: Trip | null) => void; selectedWinery: Winery | null; setSelectedWinery: (winery: Winery | null) => void; proposedWinery: Winery | null; setProposedWinery: (winery: Winery | null) => void; trulyDiscoveredWineries: Winery[]; visitedToRender: Winery[]; wishlistToRender: Winery[]; favoritesToRender: Winery[]; }) {
  ...
  }, [filter, allPersistentWineries, trulyDiscoveredWineries, mapBounds, selectedTrip]);
    ...
    return (
        <APIProvider apiKey={apiKey} libraries={['places', 'geocoding', 'marker']}>
            <WineryMapLogic 
                ...
                proposedWinery={proposedWinery} 
                setProposedWinery={setProposedWinery} 
                trulyDiscoveredWineries={trulyDiscoveredWineries} 
                visitedToRender={visitedToRender} 
                wishlistToRender={wishlistToRender} 
                favoritesToRender={favoritesToRender} 
