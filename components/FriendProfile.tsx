...
export default function FriendProfile({ friendId }: FriendProfileProps) {
  ...
  const profile = selectedFriendProfile?.profile;
  const visits = selectedFriendProfile?.visits || [];
  const stats = selectedFriendProfile?.stats || { visit_count: 0, wishlist_count: 0, favorite_count: 0 };

  return (
    <div 
      className="space-y-6 pb-12" 
      data-testid="friend-profile-container" 
      data-state={error ? 'error' : isLoading && !profile ? 'loading' : 'ready'}
    >
      {error ? (
        ...
        <div className="text-center py-12 bg-muted/20 rounded-xl border-2 border-dashed border-muted mt-8">
          <p className="text-muted-foreground italic text-sm">Profile not found.</p>
