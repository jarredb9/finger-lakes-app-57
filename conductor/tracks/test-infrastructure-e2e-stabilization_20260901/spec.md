...
The primary objectives are organized into **11 bounded phases** (adhering strictly to Contract Decoupling, Domain Separation, and Bounded Phase Sizing of max 3–4 tasks per phase):
...
- **QA-16 (Granular Domain Handler Decomposition & Browser Shimming - Phase 7)**:
  - Decompose monolithic `e2e/fixtures/maps.fixture.ts` (800 lines) and `e2e/fixtures/trips.fixture.ts` (680 lines) into focused sub-handlers, browser shims, and shared utilities:
    - `e2e/fixtures/utils/mock-wineries.ts`: Centralizes canonical `MOCK_MARKERS` and `getEquivalentWineryIds` normalization.
    - `e2e/fixtures/utils/diagnostic-logger.ts`: Encapsulates network logging buffers, console error suppressions, and diagnostic flushing.
    - `e2e/fixtures/shims/browser.shim.ts`: Encapsulates WebGL constants, Canvas context proxying, Mapbox WebGL spoofing, and service worker registration filtering.
    - `e2e/fixtures/shims/google-maps-sdk.shim.ts`: Encapsulates Google Maps JS SDK stubbing.
    - `e2e/fixtures/handlers/places.handler.ts`: Encapsulates Places REST v1, legacy place endpoints, and Edge Functions.
    - `e2e/fixtures/handlers/assets.handler.ts`: Encapsulates tiles, fonts, Mapbox styles/tiles, and Open-Meteo weather mocks.
    - `e2e/fixtures/handlers/trips.handler.ts`: Encapsulates trips RPCs, member invites, and `/rest/v1/trips`.
    - `e2e/fixtures/handlers/visits.handler.ts`: Encapsulates visits RPCs, photo uploads, idempotency keys, and paginated visits.
    - `e2e/fixtures/handlers/social.handler.ts`: Encapsulates friend requests, friend graph, activity feeds, and profile stats.
    - `e2e/fixtures/handlers/favorites.handler.ts`: Encapsulates favorite/wishlist toggles, privacy RPCs, and dynamic marker decoration.
  - Refactor `MapsFixtureManager` in `maps.fixture.ts` (<80 lines) and `TripsFixtureManager` in `trips.fixture.ts` (<85 lines) into thin orchestrator facades delegating to these handlers with 100% backward compatibility.
...
- **QA-10 (High-Value E2E Coverage Gaps - Phase 10)**:
  ...
  - Conduct strict `any` type audits across Phase 10 spec files (`e2e/trip-management.spec.ts`, `e2e/pwa-offline.spec.ts`) and associated fixture handlers (`e2e/fixtures/handlers/trips.handler.ts`, `e2e/fixtures/handlers/visits.handler.ts`), replacing ad-hoc casts with augmented global `Window` properties (`window._E2E_FULL_DRAWER`, `window.use*Store`) and canonical domain interfaces (`RpcVisitWithWinery`, `MockTrip`, `Winery`, `Route`).
- **Lint Guardrails**:
